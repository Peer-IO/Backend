...
export const createSubmission = async (req, res) => {
	const user = req.user;
	const course = await courseCrud.getOneDoc({
		findBy: { _id: req.body.course },
	});
	...
	const assignment = await assignmentCrud.getOneDoc({
		findBy: { _id: req.body.assignment },
	});
	if (!assignment)
	...
	if (submissionDeadline >= new Date()) {
		try {
			const submission = await submissionCrud.createOne({
				body: { ...req.body, submitter: user._id, reviews: [] },
			});
...
export const getSubmission = async (req, res) => {
	try {
		...
		if (
			assignmentId &&
      !(await assignmentCrud.getOne({ findBy: { _id: assignmentId } }))
		) {
		...
		if (userSpecific === "T") {
			...
			if (!courseId && !assignmentId) {
				...
				if (req.user.teacher) {
					...
					return res
						.status(400)
						.json({ error: "Only available to instructor." });
				}
			}
		} else {
			res.statusCode = 400;
		...
		if (submission) {
			...
			if (submitterId === userId) {
				const assignment = await assignmentCrud.getOne({
					findBy: { _id: submission.assignment },
				});
				if (isSubmissionUpdatable(assignment)) {
					...
					return res
						.status(403)
						.json({ error: "Submission Deadline is over." });
				}
			} else {
				return res
					.status(400)
					.json({ error: "Only submitter can delete the submission." });
			}
		} else {
...
export const getSingleSubmission = async (req, res) => {
	try {
		...
		if (submission) {
			...
			return res
				.status(404)
				.json({ error: "No submission for given Id exists." });
		}
	} catch (err) {
		return res.status(400).json({ error: err.message });
	}
...
export const updateSubmission = async (req, res) => {
	try {
		const submission = await submissionCrud.getOne({
		...
		if (submitterId === req.user._id.toString()) {
			...
			if (assignment) {
				if (
					submission &&
          assignment._id.toString() === submission.assignment.toString()
				) {
					if (isSubmissionUpdatable(assignment)) {
						const updatedSubmission = await Submission.findByIdAndUpdate(
							{ _id: req.params.id },
							{ $set: { attachments: req.body.attachments } },
							{ new: true }
						);
					...
					return res
						.status(400)
						.json({ error: "Submission is not for given assignment." });
				}
			} else {
			...
			return res
				.status(400)
				.json({ error: "Only submitter can update the submission." });
		}
	} catch (err) {
		console.log(err);
		return res.status(400).json({ error: err.message });
...
export const getReviewedSubmissions = async (req, res) => {
	if (req.user.teacher) {
		const assignment = await assignmentCrud.getOne({
			findBy: { _id: req.query.assignmentId },
		});
		const courseId = req.query.courseId;
		if (assignment) {
			if (assignment.course.toString() === req.query.courseId) {
				...
				return res
					.status(400)
					.json({ error: "Course is not of provided assignment." });
			}
		} else {
